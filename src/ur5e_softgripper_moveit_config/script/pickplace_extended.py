...
class PickPlace:
	...
	def SortContainers(self):
		...
		for container in self.tp.container_pose_list:
			...
			if boundingbox_area < 1500:
				continue
			elif boundingbox_area < 1650:
				container.results.Class = "small_container"
			elif boundingbox_area < 2000:
	...
	def SourceTable_Object_Low(self, obj, orientation):
	...
	def SourceTable_Object_Shelf(self, obj):
	...
	def SourceTable_Object_High(self, obj, orientation):
	...
	def Pick_Shelf(self, depth, obj):
		...
		pickpose.position.z = depth + 0.195
	...
	def Pick_High(self, depth, obj):
		...
		if obj == 'carrot_slice':				
			pickpose.position.z = depth + 0.2 #Adjust according to the height
		else:
				...
				while place_count[obj] < pick_count[obj]:
					for detected_obj in self.tp.detected_obj_pose_list: 
						if detected_obj.results.Class == obj:
							if obj == 'broccoli' or obj == 'carrot_slice' or obj == 'cookies' or obj == 'fried_eggs' or obj == 'iced_gems' or obj == 'meatballs' or obj == 'noodles':

								orientation = "Perpendicular"
								self.ServoOrientation(orientation)

							elif obj == 'Green_Beans' or obj == 'Sausage':
							...
							if obj == 'bottle':
								...
								self.Pick_Low(self.source_object_depth) 
								self.AssemblyTable()
							...
							for pose in plate_list:
								...
								if obj == 'noodles' or obj == 'meatballs' or obj == 'Sausage' or obj == 'Green_Beans':
				...
				while place_count[obj] < pick_count[obj]:
					for detected_obj in self.tp.detected_obj_pose_list: 
						if detected_obj.results.Class == obj:
							...
							self.SourceTable_Object_High(detected_obj, orientation)
	...
	def PnP_Location_Shelf(self, plate_list, pick_count, place_count, location):
		...
		enter_shelf = self.tp.arm_group.get_current_pose()
		enter_shelf.position.x -= 0.05
				...
				while place_count[obj] < pick_count[obj]:
					for detected_obj in self.tp.detected_obj_pose_list: 
						if detected_obj.results.Class == obj:
							...
							self.SourceTable_Object_Shelf(detected_obj, orientation)
							self.Pick_Shelf(self.source_object_depth, obj) 
							self.AssemblyTable()
					...
					for detected_obj in self.tp.detected_obj_pose_list: 
						if detected_obj.results.Class == obj:
							...
							for pose in plate_list:
								...
								if obj == 'Cookies' or obj == 'iced_gems':
									if pose.results.Class == 'rect_container':
...
if __name__ == '__main__':
	...
	location_1 = [-0.04778, -2.05435, 1.88519, -1.3745 ,-pi/2, 1.4936] ##Front left
	# location_2 = [-0.8999, -1.9786, 1.84452 -1.42536, -1.59678, 0.64132] ##Front right
	# location_3 = [0.10883, -1.198, 0.2748, -0.5993 ,-pi/2, 1.66194] ##Top left
	# location_4 = [-0.45686, -1.3046, 0.44508, -0.666 ,-pi/2, 1.0968] ##Top right
	# location_5 = [-1.1348, -1.66877, 1.66492, -1.50768, -1.61135. 0.41078] ## Bottle

	pick_count_1 = {'Green_Beans': 1}
	# pick_count_3 = {'carrot_slice': 1}
	# pick_count_4 = {'broccoli': 1}

	yellow_place_count_1 = {'Green_Beans': 0}
	yellow_place_count_3 = {'carrot_slice': 0}
	yellow_place_count_4 = {'broccoli': 0}
